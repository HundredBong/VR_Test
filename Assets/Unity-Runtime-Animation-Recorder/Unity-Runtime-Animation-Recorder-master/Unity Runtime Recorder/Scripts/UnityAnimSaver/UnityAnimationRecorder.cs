...
public class UnityAnimationRecorder : MonoBehaviour {
    ...
    [Tooltip("How many frame to skip before adding frame ? Increase to optimize;"),Range(0,10)]
    public int skipFrame = 3;
    private int currentSkipFrame = 0;
    public LayerMask recordingLayer;
    [HideInInspector]
    public AnimationClip lastMadeClip;
	...
	void SetupRecorders () {
        ...
        List<Transform> recordList = new List<Transform>();
        recordObjs = gameObject.GetComponentsInChildren<Transform>();
        foreach (Transform child in recordObjs)
        {
            // ...check if layer in recording layermask
            if (recordingLayer == (recordingLayer | (1 << child.gameObject.layer)))
            {
                recordList.Add(child);
            }
        }

        recordObjs = recordList.ToArray();
